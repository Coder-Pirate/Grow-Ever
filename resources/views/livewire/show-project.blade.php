        ...
        <div class="container">
            <div class="row">
                <div class="col-8 mx-auto text-center">
                    <h2 class="mb-3 text-capitalize">Project</h2>
                    <ul class="list-inline breadcrumbs text-capitalize" style="font-weight:500">
                        <li class="list-inline-item"><a href="{{ route('home') }}">Home</a>
                        </li>
                        <li class="list-inline-item">/ &nbsp; <a href="">Project</a>
                        </li>
                    </ul>
                </div>
            </div>
                            ...
                            <div class="col-md-6" data-aos="fade">
                                <article class="blog-post">
                                    <div class="post-slider slider-sm rounded">
                                        ...
                                        <img loading="lazy" decoding="async" src="{{ asset('storage/'.$project->image) }}" alt="Post Thumbnail">
                                        @endif
                                        
                                    </div>
                                    <div class="pt-4">
                                        <p class="mb-3">{{ \Carbon\Carbon::parse($project->created_at)->format('d M, Y') }}</p>
                                        <h2 class="h4"><a class="text-black"wire:navigate href="{{ route('projectDetail',$project->id) }}">{{ $project->title }}</a></h2>
                                        
                                        <a wire:navigate href="{{ route('projectDetail',$project->id) }}" class="text-primary fw-bold" aria-label="Read the full article by clicking here">Read More</a>
                                    </div>
                                </article>
                            </div>
                        @endforeach  
                ...
                <div class="col-lg-3">
                    ...
                    <div class="widget widget-categories">
                        ...
                        <ul class="list-unstyled widget-list">	
                            ...
                            @if ($iteams->isNotEmpty())
                                @foreach ($iteams as $iteam)
                                    
                               
                            
                            <li><a wire:navigate href="{{ route('project').'?iteamSlug='.$iteam->slug}}">{{ $iteam->name  }} </a>
                            </li>	
                            @endforeach		
                            @endif
                        </ul>
                                ...
                                <li class="d-flex widget-post align-items-center">
                                    <a class="text-black" href="/blog/elements/">
                                        <div class="widget-post-image flex-shrink-0 me-3">
                                            <img class="rounded" loading="lazy" decoding="async" src="{{ asset('storage/'.$latestProject->image) }}" alt="Post Thumbnail">
                                        </div>
                                    </a>
                                    <div class="flex-grow-1">
                                        <h5 class="h6 mb-0"><a class="text-black" wire:navigate href="{{ route('projectDetail',$latestProject->id) }}">{{ $latestProject->title }}</a></h5>
                                        <small>{{ \Carbon\Carbon::parse($latestProject->created_at)->format('d M, Y') }}</small>
                                    </div>
                                </li>
                            </ul>
                            @endforeach
